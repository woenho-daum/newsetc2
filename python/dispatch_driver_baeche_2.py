...
def save_to_sqlite(conn, query_date: str, office: str, run_flag: str, week_flag: str,
    ...
    # 재실행 시 동일 조회일자 데이터는 삭제 후 재적재 (멱등성 확보)
    cur.execute("DELETE FROM dispatch_summary WHERE query_date = ?", (query_date,))
    cur.execute("DELETE FROM dispatch_daily WHERE query_date = ?", (query_date,))
    ...
    """, [
        {"query_date": query_date, **r} for r in daily_rows
    ])
...
def load_from_html_file(html_path: str, query_date: str, office: str, run_flag: str,
    ...
    save_to_sqlite(conn, query_date, office, run_flag, week_flag, summary_rows, daily_rows)
    conn.close()
    return len(summary_rows), len(daily_rows)
...
def main():
    ...
    if args.from_html:
        n_summary, n_daily = load_from_html_file(
            args.from_html, args.date, args.office, args.run_flag, args.week_flag, args.db
        )
        print(f"[완료] HTML 파일에서 적재: 직원 {n_summary}명, 일자별 레코드 {n_daily}건 -> {args.db}")
        return
    ...
    save_to_sqlite(conn, args.date, args.office, args.run_flag, args.week_flag, summary_rows, daily_rows)
    conn.close()

    print(f"[완료] 직원 {len(summary_rows)}명, 일자별 레코드 {len(daily_rows)}건 적재 -> {args.db}")
