...
collect_dispatch_settings.py 를 실행한다.
"""
...
def run_collect_script():
    print(f"[정보] {COLLECT_SCRIPT_NAME} 실행 중...")
    collect_dispatch_settings.main()   # collect 쪽 함수명에 맞게 수정
    print(f"[정보] {COLLECT_SCRIPT_NAME} 실행 완료")
...
COLLECT_SCRIPT_NAME = "collect_dispatch_settings.py"
...
def run_collect_script():
    """같은 폴더의 collect_dispatch_settings.py 를 실행하고 종료될 때까지 대기한다."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    script_path = os.path.join(script_dir, COLLECT_SCRIPT_NAME)

    if not os.path.exists(script_path):
        print(f"[경고] {COLLECT_SCRIPT_NAME} 파일을 찾을 수 없습니다: {script_path}")
        return

    print(f"[정보] {COLLECT_SCRIPT_NAME} 실행 중...")
    result = subprocess.run([sys.executable, script_path], cwd=script_dir)
    if result.returncode != 0:
        print(f"[경고] {COLLECT_SCRIPT_NAME} 가 비정상 종료되었습니다 (code={result.returncode})")
    else:
        print(f"[정보] {COLLECT_SCRIPT_NAME} 실행 완료")
